...
Lets get some general knowledge about arduno IDE preview.
Below picture shows the startup preview of the arduino IDE. in the that preview you have your coading sketch.at the to side theis a menya which have serveral options

.. image:: https://www.theengineeringprojects.com/wp-content/uploads/2018/10/Introduction-to-Arduino-IDE-9.jpg


Lets look at option in the arduino meun toolbar
-File- as its name suggets this you ca do the documentary things with this option. you can open new sketch,close r save that sketch ,open existiong sketch or example and change  system prefeeferce with this
-Edit- with thi option yo can change your font sizes indentations and som writing options
-sketch- this is important. compilatiation ,uploading and library options are available in this option.
tools- hardware setup is done in this option. you can change the board type and programmer type and connect port name with this.
help- this option i sused for connect with arduino referces and gwttig help.

below this menu you have som shortcut keys.first one is compie key button. by using this you can know your code is gramerly correct or notacocrding to coding rules. using upload button you can do both comple and uplaod preocess in same time. 
the right  top corener have serial mitor. this is very important imption.this use to communicat eadruino ide and you board . you can sen input and watch the data sent by arduinon from this . undee serial monitor button we can see hte some small icom option .this option is used to configure the new tab configratin.
at the bottom side you can see the massage colum .thses shows result of the cpmilation and uploading and sm  othe massages.

in the sketch wecan write any arduino code.this our lplaground. we can write any relat code in that space. in that sapce the are two sections. on is void setp and othe is void loop.i the void setup we wryte which we want to exete at stsrt at one time. so we wrt pincnfigurations and othe library configurations. our mainalgorthm is writein void loop.as is name sufgeggst the pocess execute this loop section repeatedly at every time. so we write the ehing we=hich we want execute at ever time in this section.
after wrteie the got to tools->selsct your board type adn selct yue port ame correlt.then click upload button to uploasd the code.
